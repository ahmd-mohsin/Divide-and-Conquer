...
Processes datasets through decomposition and chain expansion pipeline.
"""
import sys
import os
...
try:
    from division.data_loaders import create_loader
...
def batch_generate_chains(
    ...
    Args:
        dataset_name: Dataset to use ('hendrycks_math', 'minif2f', 'proofnet', 'deepmind')
    ...
    problems = loader.load(max_problems=num_problems, category=category)
    ...
    for i, (problem, answer, metadata) in enumerate(problems, 1):
        ...
        if verbose:
            ...
            print(f"  Category: {metadata.get('type', 'Unknown')}")
        ...
        try:
            ...
            if not os.path.exists(prompts_file):
                ...
                if os.path.exists(division_prompts):
                    ...
                    parent_prompts = os.path.join(parent_dir, "hcot_prompts.json")
                    if os.path.exists(parent_prompts):
                        prompts_file = parent_prompts
            ...
            # Step 2: Generate hierarchical execution chains
            if verbose:
                print(f"\n  Step 2: Generating hierarchical execution chains...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate hierarchical reasoning chains for math problems",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process Hendrycks MATH dataset
  python batch_chain_generation.py --dataset hendrycks_math --num-problems 10
  
  # Process specific category
  python batch_chain_generation.py --dataset hendrycks_math --category Algebra --num-problems 20
  
  # Generate more execution chains per problem
  python batch_chain_generation.py --dataset hendrycks_math --num-chains 10 --num-problems 5
  
  # Use higher temperature for more diversity
  python batch_chain_generation.py --dataset hendrycks_math --temperature 1.0 --num-problems 10
        """
    )
    ...
    parser.add_argument("--generation-model", default="llama3.1:latest",
                       help="Model for chain generation")
    ...
    parser.add_argument("--delay", type=float, default=1.0,
                       help="Delay between problems (seconds)")
    ...
    print("\n" + "="*70)
    print("HCOT MODULE 2: Chain-of-Thought Generation")
    print("="*70 + "\n")
    ...
    # Print summary
    print("\n" + "="*70)
    print("BATCH CHAIN GENERATION COMPLETE")
    print("="*70)
    print(f"Time: {elapsed:.1f}s ({elapsed/60:.1f} minutes)")
    print(f"Success: {stats['total_successful']}/{stats['total_attempted']}")
    print(f"Failed: {stats['total_failed']}")
    print(f"Total chains generated: {stats['total_chains_generated']}")
    print(f"Avg chains per problem: {stats['total_chains_generated']/stats['total_successful']:.1f}")
